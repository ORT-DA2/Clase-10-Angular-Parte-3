...
### 3) Inyectamos el servicio en nuestro PetListComponent

La inyección la logramos a través del constructor de la clase, para ello hacemos en ```pet-list.component.ts```:
...
Eso el PetService y lo deja disponible para la clase. Ahí mismo podríamos inicializar nuestras pets, llamando al ```getPets``` del servicio, sin embargo, no es proljo mezclar la lógica de construcción del componente (todo lo que es renderización de la vista), con lo que es la lógica de obtención de datos. ¿Qué otra forma vimos hace un ratito para resolver este tipo de casos? Los lifecycle hooks, particularmente, el ```OnInit``` que ya definimos. Entonces hacemos:
